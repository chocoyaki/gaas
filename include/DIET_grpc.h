...
extern "C" {
  ...
  struct grpc_function_handle_s {
    ...
    diet_profile_desc_t profile; /*!< profile description for the corresponding server */
...
#define grpc_error_t                  diet_error_t
  ...
  /**
     Function used to read the configuration file and initialize the required modules.
  ...
  */
  ...
  /**
     ...
     @return error value specifying whether the finalization was successful or not: 
     \arg GRPC_NO_ERROR on success.
     \arg GRPC_NOT_INITIALIZED if a call to grpc_initialize() has not been performed successfully or at all.
  ...
  */
  ...
  /**
     Function used to allocate a new function handle using a default server assoicated with the
  ...
  */
  ...
  /**
     Function used to allocate a new function handle with a server explicitly specified by the
     user. This explicit server is specified by a string of the form
     "host:port".
         
     @warning THIS FUNCTION IS STILL NOT IMPLEMENTED.                     
     @param handle function handle to allocate
     @param server_host_port This explicit server is specified by a string of the form "host:port"
  ...
  */
  ...
  /**
     Function used to release all information and resources associated with the specified
     function  handle. 
  ...
  */
  ...
  /**
     Function used to return the function handle corresponding to the given session ID. 
         
     @param handle function handle to retrieve
     @param sessionID the id of the session of which we want to get a function handle
     @return error value telling whether the call was sucessful or not:
     \arg GRPC_NOT_INITIALIZED if a call to grpc_initialize() has not been performed successfully or at all
  ...
  */
  ...
  /** 
      ...
      @param maxsize is the maximum number of arguments that can be pushed onto this stack.                                     
  ...
  */
  ...
  /** 
      Function used to push the argument onto the stack. If the push operation exceeds the
  ...
  /** 
      Function used to remove the top element of the stack. If the stack is empty, return NULL.
  ...
  */
  ...
  /**
     ...
     @remark The GridRPC way of passing arguments is not DIET-compliant, since it
     ...
     @return error value telling whether the call was sucessful or not:
     \arg GRPC_NO_ERROR on success
     \arg GRPC_NOT_INITIALIZED if a call to grpc_initialize() has not been performed successfully or at all, if the initialization failed, or if the MA/MADAG/Workflow LogService was not located
  ...
  */
  ...
  /**
     Function used to make a non-blocking remote procedure call with a variable number of
  ...
  */
  ...
  /** 
      Function used to make a non-blocking call using the argument stack. Similarly, a session
  ...
  */
  ...
  /**
     Function used to check an array of session IDs for any GridRPC calls that have completed.
  ...
  */
  ...
  /**
     Function used to block until all of the specified non-blocking requests in a given set
     have completed. 
  ...
  */
  ...
  /**
     Function used to block until any of the specified non-blocking requests in a given set
     have completed. Exactly one session ID is returned in idPtr.
     ...
     @todo status_to_grpc_code() must be used to convert status codes to grpc error codes inside the underlying diet call
  ...
  */
  ...
  /**
     Function used to block until all previously issued non-blocking requests have completed
         
     @return error values specifying whether the calls have been successfully completed:
     \arg STATUS_ERROR if something went wrong when waiting
     \arg STATUS_DONE if the call has completed
     @todo status_to_grpc_code() must be used to convert status codes to grpc error codes in the underlying diet call 
  */
  grpc_error_t
  grpc_wait_all();
  ...
  /**
     Function used to block until any previously issued non-blocking requests have completed.
     Exactly one session ID is returned in idPtr.
...
#define grpc_perror                   diet_perror
  ...
  /**
     Function used to return the error description string, given a GridRPC error code.
...
#define GRPC_NOT_INITIALIZED 1
  /**
     Error code specifying that the config file to be use for the initialization
     of the GRPC or DIET was not found
  */
#define GRPC_CONFIGFILE_NOT_FOUND 2
  /**
     Error code specifying that there was an error when reading the config file
         
     @todo during the parsing this error code could be used or some more precise error code that would be the equivalent of DIET_FILE_IO_ERROR for example
  */
#define GRPC_CONFIGFILE_ERROR 3
  /**
     Error code specifying that no server was found to solve the provided problem
  */
...
#ifdef HAVE_WORKFLOW
  /**
     Error code specifying that the provided XML file describing the workflow was malformed
  */
