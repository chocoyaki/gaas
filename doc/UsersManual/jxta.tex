...
\item{Download from
    ...
    version of DIET$_{JXTA}$ is based on JXTA 2.3). Files to download
    are :}
\begin{itemize}
\item{\texttt{jxta.jar},}
...
\item{\textbf{First step}: launching a MA$_{JXTA}$. After having set
    ...
    \begin{itemize}
    \item{Ensure that \texttt{JXTA\_LIB}, the directory containing the
        JXTA JARs, is correctly set.}
...
\item{\textbf{Second step}: registering a SeD to the MA. Be sure that
    ...
    \texttt{\$ java -cp <JXTA\_JARS> JXTASeD
...
\item{\textbf{Third step}: Launch a JXTA client with the command:\\\\
                    \texttt{\$ java -cp <JXTA\_JARS> ClientJXTA <pb>}}
