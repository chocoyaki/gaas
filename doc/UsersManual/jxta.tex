...
algorithm of discovery. The DIET$_{JXTA}$ is shown Figure
...
\begin{figure}[htb]
 \begin{center}
  ...
  \label{fig:platform}
  \caption{DIET$_{JXTA}$ architecture}
 \end{center}
\end{figure}
...
DIET$_{JXTA}$ components having a DIET part and a JXTA part are the MA
and the SeD.
...
\section{The future of DIET$_{JXTA}$}
\label{sec:future}
...
\item{An unsolved problem dealing with \textbf{omniORB and JNI}
    ...
    between a DIET$_{JXTA}$ MA and a DIET$_{JXTA}$ SeD, they have to
    be launched via JNI.  Moreover, a DIET$_{JXTA}$ MA won't be able
    to know LAs or SeDs not launched via JNI.  The current
    DIET$_{JXTA}$ tree is unable to contain classic LAs$_{DIET}$ or
    SeDs$_{DIET}$.}
  
\item{The current version of the DIET$_{JXTA}$ platform works only for
...
\section{Working with a DIET$_{JXTA}$ platform}
...
\item{Then, to configure DIET and to deploy DIET$_{JXTA}$ platforms, the
    ...
    scheduling is made in the current DIET$_{JXTA}$ platform:
...
\subsection {Deploying a DIET$_{JXTA}$ platform}
...
\begin{itemize}
  ...
  \item{\textbf{First step}: launching a MA$_{JXTA}$. After having set
...
\item{Each time a new JXTA peer is launched, you have to configure
      ...
      settings. When using DIET$_{JXTA}$ on a single machine, the
      configuration is as shown on figure 7.2, else, just replace
      \texttt{localhost} by the IP address of the machine.  Please
      note that, for each peer on a single machine, the TCP and HTTP
      ports have to be different. For instance : 9701 and 9700 for
      the first peer, 9703 and 9702 for the second, etc. The third
...
\item{\textbf{Third step}: Launch a client$_{JXTA}$ with the command:\\
    {\footnotesize
