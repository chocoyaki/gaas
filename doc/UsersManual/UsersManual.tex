...
\begin{tabular}{ll}
  ...
  \textbf{PROJECT MANAGER}  & Frédéric \textsc{Desprez}.\\
...
\begin{minipage}[t]{12cm}
  ...
  Sylvain \textsc{Dahan}, Bruno \textsc{Delfabro}, Georg \textsc{Hoesch}, Mathieu \textsc{Jan}, Christophe \textsc{Pera}, Cyrille \textsc{Pontvieux}, Cédric \textsc{Tedeschi}, Jean-Yves \textsc{L'Excellent} and Antoine \textsc{Vernois}.
\end{minipage} \\
...
servers by submitting jobs to dedicated servers. This model is known
...
paradigm. On a Grid platform, the RPC (or
GridRPC~\cite{MNS+00,NMSDLC02}) offers an easy access to available
resources to a Web browser, a Problem Solving Environment (PSE), or a
simple client program written in C, Fortran, or Java.  It also
provides more transparency by hiding the search and allocation of
computing resources. We favour this second approach.

In a Grid context, this approach requires the implementation of
middlewares to facilitate the client access to remote resources. In
...
the schedulers of this toolbox. In DIET, user's requests are served
via RPC.
