...
differences occur between the DIET and the FAST profile). This is not an easy
task and will therefore be explained separately in the next paragraphs.

%%%%%%%%%
%% FIXME: Check this part by time ...
%%%%%%%%%

\subsubsection*{Using convertors}

The service profiles offered by DIET are sometimes not
understandable by the implementation. To solve this problem,
...
default convertor is assigned to it that does neither change its
...
This is done implicit by specifying which argument in the source
...
then be used as value for the destination argument. Corresponding to
...
\noindent Lets look a a short example:
\footnotesize
\begin{verbatim}
