...
%* $Id$
%* $Log$
%* Revision 1.26  2006/10/09 21:17:10  eboix
%*    First coarse version of cmake documentation. --- Injay2461
%*
...
and \scalapack\ libraries. However the compilation of those BLAS dependant
...
\verb+cmake+'s builtin regression tests mechanism).
...
Optionaly, you can download the sources dans recompile them: this simple
...
DIET respects \verb+cmake+'s best pratices e.g.~by clearly separating the
...
\verb+Cmake+ classicaly provides two ways for setting configuration
...
specifes a path to the top level of the source tree (i.e. where the directory
...
the project (out of source building is generaly encouraged specially
...
\item
  ...
  \verb+Debug+ will produce binaries and libraries with the debugging
   information
\item
   \verb+CMAKE_VERBOSE_MAKEFILE+ is a booelan paramater which when set to
   ON will generate makefiles without the .SILENT directive. This is
   usefull for watching the invoked commands and their arguments in case
   things go wrong.
\item
   \verb+CMAKE_C[XX]_FLAGS*+ is a familiy of paramaters used for
...
\item
  ...
  sumarises \verb+ccmake+'s most pertinent options (corresponding
...
\item
  ...
  lines, hit "e" to exit)
\item
  up/down navigation among paramater items can be achieved with the
  up/down arrows
\item
...
source level distribution tarball, or for DIET developpers corresponds to
the directory GRAAL/devel/diet/diet of a cvs checkout of the DIET sources
hierarchy.
Additionaly, assume we created a build tree directory and \verb+cd+
...
\item
  ...
  \item check all the paramaters preceeded with the * (star) character
...
%==================
\subsubsection{DIET's optional configuration flags}
...
\item
  ...
  version of your \LaTeX\ compiler and it also relies on many subdepencies
  (e.g. \verb+doxygen+ or \verb+fig2dev+)
\item
  \verb+DIET_BUILD_EXAMPLES+ for building various client/server examples
\item
  \verb+DIET_USE_BATCH+ for ... Appleseeds based batch extensions.
\item
  \verb+DIET_USE_BLAS+ for ... BLAS based examples.
\item
  \verb+DIET_USE_CORI+ for ... a built-in low-profile FAST surrogate.
\item
  \verb+DIET_USE_FAST+ for ... using FAST a grid aware dynamic
  forecasting library.
\item
  \verb+DIET_USE_JXTA+ for ... using JXTA for dynamic MULTI Master
  Agent support.
\item
  \verb+DIET_WITH_MULTI_MA+ for ... MULTI Master Agent based support.
\item
  \verb+DIET_WITH_STATISTICS+ for ... good question, for what ?
\end{itemize}
