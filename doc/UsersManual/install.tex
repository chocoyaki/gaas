...
on Solaris/Sparc, Linux/Sparc, Linux/i64, Linux/amd64 and Linux/Alpha platform and
seems be supported.
...
\begin{itemize}
 ...
 \item{\textbf{omniORB 4}} which depends on \textbf{Python 2.1} or later,
                           ...
                           platform to be secure.~\footnote{If you want to deploy
			   a secure DIET platform, SSL support is not yet implemented 
			   in DIET, but an easy way to do so is to deploy DIET on a VPN.}
...
as a sub-module of examples (which means that this option has no
effect if examples are disabled) - disabled by default.
...
\noindent This option allows the user to deploy DIET$_{JXTA}$ architectures. 
When this option is activated (disable by default), DIET allows only
this type of architecture to be deployed.
...
\noindent This option allows the user to connect several MA to be linked
together. When this option is activated, it allow a MA to looking for a SeD
into the other MA hierarchies.
