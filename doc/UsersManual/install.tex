...
\\
\textbf{NB:} these options has no effect if the module example and/or
...
\item{\texttt{file\_transfer}}: the server computes the sizes of two
  input files and returns them (with or without, randomly, the first
  file) as output arguments to the client.
...
\item{\texttt{BLAS}}: the server offers the dgemm BLAS functionality.
...
\item{\texttt{\scalapack}}: the server is designed to offer all \scalapack\ 
  ...
  \texttt{pdgemm\_} function so far. The \texttt{pdgemm\_} routine is actually the
