...
\end{verbatim}
This function replaces the set fucntions (see Section \ref{sec:setfun}).



\begin{itemize}
\item[NB:] a mecanisms of publication like isnt implemented yet. So,
exchanges of identifiers between clients must be explicitly done..
\end{itemize}
...
twice, we cannot know what will be its identifier. So, rather than
explictly use the identifier when can use the structure of the data.
For example, if A matrix is generated by a  
{\footnotesize
\begin{verbatim}
...
...
...
\end{verbatim}
} method, we can use A in the same session by writing
{\footnotesize
\begin{verbatim}
...
...
...
\end{verbatim}
} with \texttt{profile2} the new profile.

\begin{itemize}
\item[NB:] when using this method, the first profile (here \texttt{profile}) must not be freed.
\end{itemize}