...
Of course, in a Makefile (or Doxyfile, shell, etc.) the '\%' sign is replaced by
a '\#'; in a C or C++ file, it is replaced by a '/'.
...
producing software collabortatively and efficiently. Comments within the source
code are one way to create a comprehensive documentation. This insures that the
related information is in the right place and makes it easier for the software
engineer to maintain it.
...
\addcontentsline{toc}{subsection}{Doxygen}
\subsection*{Doxygen}
Doxygen comments can be extracted from the source code by the doxygen tool. The
doxygen tool identifies the different sections inside an doxygen comment and
creates I do not what but Sylvain will fill in this part of the Coding Standards
with a complete example, so that all of us will be able to use Doxygen basics
very soon.\\


As there are C files in the DIET sources, the flag \texttt{@author} cannot be
used in all DIET source files. Thus, it is important to authentify the
author of the file \textbf{in the header}, as explained section \ref{fstruct}.
Using the flag \texttt{@author} would be redundant.
