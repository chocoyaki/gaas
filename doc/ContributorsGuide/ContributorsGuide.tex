...
\hypersetup{ % Modifiez la valeur des champs suivants
    pdfauthor   = {Eddy Caron, Yves Caniou, Benjamin Depardon, Haïkel Guémar}
...
\newcommand{\todohg}[2][]{\todoinline[color=purple!50,#1]{\textbf{Haïkel:} #2\
}}
...
\textbf{\Huge CONTRIBUTOR'S MANUAL\\[2ex]}
\end{center}
...
\begin{tabular}{ll}
  ...
  \textsc{Caron}, Benjamin \textsc{Depardon} and Haikel \textsc{Gu\'emar}.\\

  \textbf{AUTHORS STAFF}    & 
\begin{minipage}[t]{12cm}
  Yves~\textsc{Caniou}, Eddy~\textsc{Caron},
  Benjamin~\textsc{Depardon} and Haikel~\textsc{Gu\'emar}.\\
\end{minipage} \\
...
\todoall{Honteusement pompé sur
  http://wiki.openbravo.com/wiki/Projects/POS/Contributor\%27s\_Guide
  mais c'est pas mal d'avoir ce genre de motivation en entrée de document.}

We truly believe that anyone can personally benefit by contributing
...
technologies like C++, CORBA, Java, \dots These are today's industry
...
\section{Participate in the mailing list and help other users}
...
\item You can fill up a feature request in the bugzilla system as a
  feature (\url{http://graal.ens-lyon.fr/bugzilla}). That's the better way. 
\item You can also send it to the diet-dev mailing list. 
\end{itemize}
\todoall{Est-ce qu'on ne dirait pas plutôt d'envoyer sur diet-usr ? Tous les
  users ne sont pas des devs, et n'ont donc pas accès à cette liste. D'ailleurs,
la liste diet-dev on ne peut pas s'y inscrire comme ça. Je suppose que seul Eddy
peut inscrire quelqu'un.}
...
the bugzilla system (\url{http://graal.ens-lyon.fr/bugzilla}) section
documentation. 

If you have created a significant piece of documentation regarding the
implementation, development or usage of \diet. You can contact at least
one member of the \dietmaint. Also you can help improve the current
documentation after registering in our GIT . 

\todoall{Est-ce qu'on fait une branche documentation ? Est-ce qu'on ouvre le GIT
comme ça juste pour de la doc ? Est-ce qu'il ne faudrait pas que ça passe
uniquement par le bugzilla ça ?}
...
\item \textbf{\dietmaint.} This group is composed of four persons: Eddy Caron
  (\href{mailto:Eddy.Caron@ens-lyon.fr}{Eddy.Caron@ens-lyon.fr}), Yves Caniou
  (\href{mailto:Yves.Caniou@ens-lyon.fr}{Yves.Caniou@ens-lyon.fr}), Benjamin
  Depardon
  (\href{mailto:Benjamin.Depardon@sysfera.com}{Benjamin.Depardon@sysfera.com}) and
  Haïkel Guémar
  (\href{mailto:Haikel.Guemar@sysfera.com}{Haikel.Guemar@sysfera.com}). The role
  of this group is to:
\begin{itemize}
  \item validate the creation of a new branch;
  \item nominate the \dietbranch for each official branch;
  \item decide with the \dietbranch if a branch  can be merged to the
    master;
...
\end{itemize}
\item \textbf{\dietcontrib.} A person who has an authorized access to
  write into a branch or into the master. His role is to:
  \begin{itemize}
    \item write and document a nice and amazing code;
    \item work in collaboration with his \dietbranch. 
\end{itemize}
 
\item \textbf{\dietguest.} He's a contributor without  GIT write
  access. He can 
\begin{itemize}
  \item  send a patch to a \dietbranch or a member of the \dietmaint;
  \item become a \diet developer as soon as possible.
\end{itemize}
...
\item \textbf{\dietpackage.} He does the merge to the master with the
  ...
  Fedora, \dots) when a new release is announced by the \dietmaint. 
\end{description}

\chapter{Becoming a \diet developer}

A \diet developer is an amazing person that is actively involved in
the development of \diet. For example, if you have a significant contribution
or write regularly enhancements for the product or planning to do it
soon.

\section{Obtaining Write Access to a GIT branch}
...
developers can write on it. You need write access permission to \diet
GIT to be able to push your code to the GIT.

\section{Regression tests}

\todobd{Des infos sur Jenkins ? Et comment ajouter de nouveau tests ou
où trouver la doc pour le faire. Récupérer la doc de \url{http://graal.ens-lyon.fr/trac/sysfera/wiki/SysFeraPrivate/ContinuousIntegration}}


\section{Coding Standard}

\todohg{Faire un lien sur le document de \diet Coding Standards fait
  par Haïkel. Est-ce qu'on garde les 15 pages ? Je veux dire par là
  que je vois bien une version pas trop rebuttante en version Lite
  pour un coder beginner puis rapidement si ça mort on lui passe le
...
platforms? We already build packages for Debian and Fedora, but any new packages
are welcome.
