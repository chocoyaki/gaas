 ...
 */
inline std::string
find_executable_in_path(const std::string &file,
                        std::string path = "") {
#if defined(BOOST_POSIX_API)
  ...
  if (path.empty()) {
    ...
    if (!envpath) {
      boost::throw_exception(std::runtime_error(
                               BOOST_PROCESS_SOURCE_LOCATION "file not found" +
                               file +
...
inline child
create_child(const std::string &executable, Arguments args,
             Context ctx) {
 ...
 */
inline child
create_child(const std::string &executable) {
...
inline child
shell(const std::string &command, Context ctx) {
#if defined(BOOST_POSIX_API)
...
#endif // ifndef BOOST_PROCESS_OPERATIONS_HPP
