...
public:
explicit
basic_status_service(boost::asio::io_service &io_service)
...
int
wait(implementation_type &impl, pid_type pid) {
...
void
async_wait(implementation_type &impl, pid_type pid, Handler handler) {
#if defined(BOOST_POSIX_API)
  boost::unique_lock<boost::mutex> lock(work_thread_mutex_);
  if (++pids_ == 1) {
...
#endif // ifndef BOOST_PROCESS_DETAIL_BASIC_STATUS_SERVICE_HPP
