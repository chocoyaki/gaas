...
DagdaImpl::sendFile(const corba_data_t &data, const char* destName) {
  ...
  unsigned long fileSize = computeFileSize(std::string(data.desc.specific.file().path));
  ...
  Transfers::getInstance()->newTransfer(std::string(data.desc.id.idNumber)+">>"+destHost,
                                        fileSize);
#endif
  ...
  for (unsigned long i = 0; i<nBlocks; ++i) {
    ...
    replace = false;
#if DAGDA_PROGRESSION
    Transfers::getInstance()->incProgress(std::string(data.desc.id.idNumber)+">>"+destHost);
#endif
  }
#ifdef USE_LOG_SERVICE
  if (getLogComponent())
...
SimpleDagdaImpl::addData(const corba_data_t& data) {
  ...
  (*getData())[std::string(data.desc.id.idNumber)].desc.dataManager = CORBA::string_dup(dataManager.c_str());
