...
public:
     ...
     */
    void enableHelp(bool showVersion=false);
...
private:
    ...
    void parseConfig();
    void findSwitch(const CmdEntry& entry);
    void findOption(const CmdEntry& entry);
    void findParam(const CmdEntry& entry);
    // used by both findOption and findParam
    const std::string findOption_(const CmdEntry& entry); 
};
...
struct CmdEntry
{
    ...
    bool operator< (const CmdEntry& other) const;
};
