...
WfValueAdapter::WfValueAdapter(const string& value)
  : myValue(value), myDataType(WfCst::TYPE_UNKNOWN), idCreatorFlag(false),
...
WfValueAdapter::WfValueAdapter(WfCst::WfDataType valueType, const string& value)
  : myValue(value), myDataType(valueType), idCreatorFlag(false),
  cx(NULL), sx(NULL), ix(NULL), lx(NULL), fx(NULL), dx(NULL), str(NULL)
{
...
WfValueAdapter::WfValueAdapter(const string& dataID,
                               ...
                               string value)
  : myDataID(dataID), myValue(value), myDataType(valueType), idCreatorFlag(false),
    cx(NULL), sx(NULL), ix(NULL), lx(NULL), fx(NULL), dx(NULL), str(NULL)  {
//   TRACE_TEXT (TRACE_ALL_STEPS,"Creating VALUE adapter (TYPE=" << valueType
//                               << ") dataID=/" << dataID << "/" << endl);
}
...
/**
 * Returns true if the adapter created the dataID itself
 * (will be false if dataID was provided in constructor)
 */
bool
WfValueAdapter::isDataIDCreator() {
  return idCreatorFlag;
}
...
WfValueAdapter::freeAdapterPersistentData(MasterAgent_var& MA) {
  // This class of adapter is NOT always the owner of its dataID because
