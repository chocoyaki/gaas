...
FConstantNode::initialize() {
  ...
  TRACE_TEXT (TRACE_ALL_STEPS, traceId() << "Set value of constant = /"
                              << myValue << "/" << endl);
  FDataTag singleTag(0,true);
  FDataHandle* dataHdl = new FDataHandle(singleTag, myOutPort->getBaseDataType(), myValue);
  myOutPort->setAsConstant(dataHdl);
}
...
FSinkNode::instanciate(Dag* dag) {
  if (isConnected) {
    ...
    while (!myIterator->isAtEnd()) {
      ...
      FDataHandle* outDH = new FDataHandle(*inDH);
      myConnectedPort->storeData(outDH);
      myConnectedPort->sendData(outDH);
    }
...
FSinkNode::getResultsInContainer(string& containerID) {
  TRACE_TEXT (TRACE_ALL_STEPS,traceId() << "Creating container of results" << endl);
