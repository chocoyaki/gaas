...
BasicWfSched::execute() {
  ...
  while (true) {
    ...
    getReadyNodes(this->myDag, readyNodes);
 ...
 */
void
BasicWfSched::getReadyNodes(Dag*& dag, vector<Node*>& readyNodes) {
	map<string, Node*> rn;
	dag->getReadyNodes(rn);
	for (map<string, Node*>::iterator p = rn.begin(); p!=rn.end(); p++) {
		readyNodes.push_back(p->second);
	} // end for
} // end getReadyNodes
