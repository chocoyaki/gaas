...
#define USERDEFINED(server, nb) (diet_est_get_internal(&(server).estim, \
                                                       EST_USERDEFINED + (nb), \
                                                       0))
#define CONSOJOB(server) (diet_est_get_internal(&(server).estim, EST_CONSOJOB, 0.0))

#define NODEFLOPS(server) (diet_est_get_internal(&(server).estim, EST_NODEFLOPS, 0.0))
...
bool						
  compConso(const corba_server_estimation_t &r1,     
    ...
    return CONSOJOB(r2) > CONSOJOB(r1);
  }

/* Sorting of the elements in descending order (The bigger element will be at the first position in the list)*/
bool						
  compPerf(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {   
    return NODEFLOPS(r2) < NODEFLOPS(r1);
  }

/* Sorting of the elements... RANDOM!*/
bool						
  compRandom(const corba_server_estimation_t &r1,     
       const corba_server_estimation_t &r2) {   
    return RANDOM(r2) > RANDOM(r1);
  }
