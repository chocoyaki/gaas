...
copyFile(char *inputName, char *outputName) {
  ...
  if ((outputFile =
         open(outputName, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR)) == -1) {
    perror("open");
    exit(-1);
  }
...
int
solve_concatenation_seq(diet_profile_t *pb) {
  size_t arg_size1 = 0;
  size_t arg_size2 = 0;
  char *prologue = NULL;
  char *copying = NULL;
  ...
  return result;
} /* solve_concatenation_seq */

/* Same code as before, but not the same pred perf */
int
solve_concatenation_parallel(diet_profile_t *pb) {
  size_t arg_size1 = 0;
  size_t arg_size2 = 0;
  char *prologue = NULL;
  char *copying = NULL;
...
main(int argc, char *argv[]) {
  ...
  if (diet_service_table_add(profile_fct1_seq, NULL,
                             solve_concatenation_seq)) {
    return 1;
  }
