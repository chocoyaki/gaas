...
class worker : public omni_thread {
  ...
  void run(void* arg) {
	...
	return;
      ...
      if (rst > 0){
	printf("debut du probe...\n");
	int state = 0;
	while ((state = diet_probe(rst)) < 2){
	  omni_thread::sleep(1);
	}
	//cout << "fin du diet_probe avec state = " <<  state << endl;
	//fflush(stdout);
	//mutexWorker.lock();
	if (pb[0]) {
	cout << "pb 0 "<< endl;
	...
	//cout << "attention CANCEL sur " << rst << endl;
	//fflush(stdout);
	diet_cancel(rst);
      }
      else {
...
main(int argc, char* argv[])
{
  int i;
  srand(time(NULL));
  DIET_DEBUG(TEXT_OUTPUT(("DEBUT du client parallelle Type 1 (diet_probe)")))
  for (i = 1; i < argc - 2; i++) {
  ...
  diet_finalize();
  DIET_DEBUG(TEXT_OUTPUT(("FIN du client parallelle Type 1 (diet_probe)")))
  return 0;
}
