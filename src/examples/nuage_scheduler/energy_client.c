/**
 * @file energy_client.c
 *
 * @brief  a DIET client displaying the least energy consuming server for transpose, MatSUM and MatPROD problems
 *
 * @author  Daniel Balouek-Thomert (Daniel.Balouek@ens-lyon.fr)
 *
 * @section Licence
 *   |LICENSE|
 */
...
main(int argc, char *argv[]) {
  ...
  for (i = 0; i < n_loops; i++) {
    ...
    if (pb[0]) {
      profile = diet_profile_alloc(path, 0, 0, 2);
      diet_matrix_set(diet_parameter(profile, 0),
                      A, DIET_VOLATILE, DIET_DOUBLE, m, n, oA);
      diet_matrix_set(diet_parameter(profile, 1),
                      NULL, DIET_VOLATILE, DIET_DOUBLE, n, m, oA);
      diet_file_set(diet_parameter(profile, 2), NULL, DIET_PERSISTENT_RETURN);
      print_matrix(A, m, n, (oA == DIET_ROW_MAJOR));
    } else if (pb[1] || pb[2]) {
      profile = diet_profile_alloc(path, 1, 1, 3);
      diet_matrix_set(diet_parameter(profile, 0),
                      A, DIET_VOLATILE, DIET_DOUBLE, m, n, oA);
      diet_file_set(diet_parameter(profile, 3), NULL, DIET_PERSISTENT_RETURN);
      print_matrix(A, m, n, (oA == DIET_ROW_MAJOR));
      if (pb[1]) {
        diet_matrix_set(diet_parameter(profile, 1),
	...
	diet_file_get(diet_parameter(profile, 3), &path, NULL, &out_size);
         if (path && (*path != '\0')) {
