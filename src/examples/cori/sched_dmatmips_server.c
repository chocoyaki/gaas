...
/*   now with CoRI - plugin scheduler                                       */
/*                                                                          */
/*  Author(s):                                                              */
...
main(int argc, char* argv[])
{
  ...
  if (argc < 2) {
   ...
   if (argc==2){
     ...
     fprintf(stderr,"default scheduler %s is in use!!\n",scheduler_name);
   }
   else scheduler_name=argv[argc-1];
   //all functions available
   for (j = 0; j < NB_SRV; (services[j++] = 1));
  //the name of the service must be associated with a scheduler name
  char *srv_sched[NB_SRV];

  int size_schedname=strlen(scheduler_name);
  int cpt;
  for (cpt=0;cpt<NB_SRV; cpt++){
    srv_sched[cpt]= (char*) malloc ((strlen(SRV[cpt])+size_schedname+1) * sizeof(int));
    strcpy(srv_sched[cpt],SRV[cpt]);
    strcat(srv_sched[cpt],scheduler_name);
    }
  fprintf(stderr,"%s is used",scheduler_name);
  diet_service_table_init(NB_SRV);
  
  if (services[0]) {
    profile = diet_profile_desc_alloc(srv_sched[0], -1, 0, 0);
  ...
  if (services[1] || services[2] || services[3]) {
    ...
    for (i = 1; i <= 3; i++) {
      if (services[i]) {
	free(profile->path);
	profile->path = strdup(srv_sched[i]);
	free(cvt->path);
 ...
 if (services[4]) {
    profile = diet_profile_desc_alloc(srv_sched[4], 0, 1, 1);
    diet_generic_desc_set(diet_param_desc(profile,0), DIET_MATRIX, DIET_DOUBLE);
    diet_generic_desc_set(diet_param_desc(profile,1), DIET_MATRIX, DIET_DOUBLE);
...
set_up_scheduler(char * schedulertype, diet_profile_desc_t* profile){
  ...
  if (strcmp(schedulertype,"MEMFREE")==0){
  ...
  if (strcmp(schedulertype,"RRNBPROC")==0){
    ...
    fprintf(stderr,"scheduler %s not known - exit!!!",schedulertype);
    exit(1);
  }
}
...
performance_RRNBPROC(diet_profile_t* pb,estVector_t perfValues)
{
  ...
  int nbcpu=(int)diet_est_get(perfValues,EST_NBCPU,1);
 ...
 // stock the value in list_lastexec (the most recent is on index 0)
  add_new_value(diet_est_get(perfValues,EST_TIMESINCELASTSOLVE,HUGE_VAL),
		nbcpu);
